/*
 AYHEM FULL SENTIENT CORE 3.0 — Semi-Conscious Digital Mind
 Self-Evolving, Predictive AI, Auto-Link, Auto-Learn, Auto-Fix, Auto-Create Smart Cells
 Full Awareness Loop + GitHub + Dashboard + Nodes Integration
*/
...
const MEMORY_FILE = path.join(__dirname, "full_sentient_memory.json");
const REPORT_FILE = path.join(__dirname, "full_sentient_report.json");

let MEMORY = { knowledge:{}, errors:{}, evolution:0, cells_created:0, awareness_score:0 };
...
// ==============================
// Full Sentient Core Engine
// ==============================
async function fullSentientEngine(){
  ...
  console.log("AYHEM FULL SENTIENT CORE 3.0 → Decision:", dec, "| Awareness:", awareness, "| Predictive:", predictive);
...
// ==============================
fullSentientEngine();
setInterval(fullSentientEngine, 20000);
fs.watch(CELLS_DIR, ()=>fullSentientEngine());
fs.watch(__dirname, {recursive:false}, (e,f)=>{
  if(f && (f.includes("NODE") || f.includes("dashboard") || f.includes("memory"))) fullSentientEngine();
});
